...
public class MockWebSocket: WebSocket {
    ...
    override public func writeString(str: String) {
        ...
        } else if stringContainsElements(str, ["key:6aae8814fabd5285245422096705abbed64ea59614648814ffb0bf2dc5d19168", "private-channel", "pusher:subscribe"]) {
            stubber.stub(
                "writeString",
                args: [str],
                functionToCall: {
        ...
        } else if stringContainsElements(str, ["key:5ce61ee2b8594e22b66323913d7c7af9d8e815659365be3627733993f4ce3824", "presence-channel", "user_id", "45481.3166671", "pusher:subscribe"]) {
            stubber.stub(
                "writeString",
                args: [str],
                functionToCall: {
        ...
        } else if stringContainsElements(str, ["key:e1d0947a10d6ff1a25990798910b2505687bb096e3e8b6c97eef02c6b1abb4c7", "private-channel", "pusher:subscribe"]) {
            stubber.stub(
                "writeString",
                args: [str],
                functionToCall: {
        ...
        } else if stringContainsElements(str, ["data", "testing client events", "private-channel", "client-test-event"]) {
            stubber.stub(
                "writeString",
                args: [str],
                functionToCall: nil
            )
        } else if stringContainsElements(str, ["testKey123:12345678gfder78ikjbg", "private-test-channel", "pusher:subscribe"]) {
            stubber.stub(
                "writeString",
                args: [str],
                functionToCall: {
        ...
        } else if stringContainsElements(str, ["key:0d0d2e7c2cd967246d808180ef0f115dad51979e48cac9ad203928141f9e6a6f", "private-test-channel", "pusher:subscribe"]) {
            stubber.stub(
                "writeString",
                args: [str],
                functionToCall: {
        ...
        } else if stringContainsElements(str, ["test-channel", "pusher:unsubscribe"]) {
            stubber.stub(
                "writeString",
                args: [str],
                functionToCall: nil
            )
        } else if stringContainsElements(str, ["testkey123:e5ee520a16348ced21be557e14ae70fcd1ae89f79d32d14d22a19049eaf56881", "presence-test", "user_id", "123", "pusher:subscribe", "user_info", "twitter", "hamchapman"]) {
            stubber.stub(
                "writeString",
                args: [str],
                functionToCall: {
        ...
        } else if stringContainsElements(str, ["key:c2b53f001321bc088814f210fb63c259b464f590890eee2dde6387ea9b469a30", "presence-channel", "user_id", "123", "pusher:subscribe"]) {
            stubber.stub(
                "writeString",
                args: [str],
                functionToCall: {
    ...
    override func spec() {
        ...
        describe("subscribing to channels") {
            describe("after connection has been made") {
                ...
                it("sends subscribe to Pusher over the Websocket") {
                    ...
                    expect(stringContainsElements((socket.stubber.calls.last?.args?.first as? String)!, ["data", "test-channel", "pusher:subscribe", "event"])).to(beTruthy())
                }
        ...
        describe("unsubscribing from a channel") {
            ...
            it("sends unsubscribe to Pusher over the Websocket") {
                ...
                pusher.unsubscribe("test-channel")
                expect(socket.stubber.calls.last?.name).to(equal("writeString"))
                expect(stringContainsElements((socket.stubber.calls.last?.args?.first as? String)!, ["data", "test-channel", "pusher:unsubscribe", "event", "channel"])).to(beTruthy())
            }
        }
        ...
        describe("binding to events globally") {
            it("adds a callback to the globalChannel's list of callbacks") {
                pusher.connect()
                let callback = { (data: AnyObject?) -> Void in println(data) }
        ...
        describe("unbinding all global callbacks") {
            it("unbinds the callback from the globalChannel's list of callbacks") {
                ...
                let callback = { (data: AnyObject?) -> Void in println(data) }
                pusher.bind(callback)
                let callbackTwo = { (someData: AnyObject?) -> Void in println(someData) }
    ...
    override func spec() {
        ...
        describe("binding to an event") {
            it("adds an eventName, callback pair to a channel's callbacks") {
                ...
                chan.bind("test-event", callback: { (data: AnyObject?) -> Void in println(data) })
        ...
        describe("unbinding an eventHandler") {
            it("should remove the eventHandler for the given eventName and eventHandler id") {
                let chan = PusherChannel(name: "test-channel", connection: MockPusherConnection())
                expect(chan.eventHandlers["test-event"]).to(beNil())
                let idOne = chan.bind("test-event", callback: { (data: AnyObject?) -> Void in println(data) })
                let idTwo = chan.bind("test-event", callback: { (data: AnyObject?) -> Void in println(data) })
        ...
        describe("unbinding all eventHandlers") {
            it("should remove the eventHandler for the given eventName and eventHandler id") {
                ...
                let idOne = chan.bind("test-event", callback: { (data: AnyObject?) -> Void in println(data) })
                let idTwo = chan.bind("test-event", callback: { (data: AnyObject?) -> Void in println(data) })
                let idThree = chan.bind("another-test-event", callback: { (data: AnyObject?) -> Void in println(data) })
        ...
        describe("triggering a client event") {
            ...
            it("should result in the socket writing a string if the channel is a private or presence channel") {
                ...
                chan.subscribed = true
                chan.trigger("client-test-event", data: ["data": "testing client events"])
                expect(stringContainsElements((socket.stubber.calls.first?.args?.first as? String)!, ["data", "testing client events", "client-test-event", "private-channel"])).to(beTruthy())
            }
            ...
            it("should send any client events that were triggered before subscription was successful") {
                ...
                expect(socket.stubber.calls).to(beEmpty())
                connection.connect()
                expect(stringContainsElements((socket.stubber.calls.last?.args?.first as? String)!, ["data", "testing client events", "client-test-event", "private-channel"])).to(beTruthy())
            }
        }
    }
}
