//
//  CryptoSwift-HMAC-sha265.swift
//  PusherSwift
//
//  Created by Hamilton Chapman on 04/04/2016.
//
//
...
public enum Authenticator {
    ...
    public func authenticate(message: [UInt8]) throws -> [UInt8] {
        ...
        case .HMAC(let key, let variant):
            guard let auth = CryptoSwift_HMAC.authenticate(key: key, message: message, variant: variant) else {
...
//    }
//    
//    init(key: [UInt8], variant: HMAC.Variant) {
//        self.key = key
...
//     */
//    public func authenticate(message: [UInt8]) throws -> [UInt8] {
//        guard let auth = HMAC.authenticate(key: key, message: message, variant: variant) else {
//            throw Error.AuthenticateError
//        }
//        return auth
//    }
...
final public class HMAC {
    ...
    class internal func authenticate(key  key: [UInt8], message: [UInt8], variant:HMAC.Variant = .sha256) -> [UInt8]? {
        return HMAC(key, variant: variant)?.authenticate(message: message)
    }
    
    // MARK: - Private
    
    internal init? (_ key: [UInt8], variant:HMAC.Variant = .sha256) {
        self.variant = variant
        self.key = key
